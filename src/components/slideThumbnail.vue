...
export default {
    name: 'slideThumbnail',
    props: ['startingImage', 'imagesSrc', 'autoSlideInterval', 'showProgressBar'],
    data() {
        return {
    ...
    computed: {
        ...
        currentImage() {
            ...
            return this.imagesSrc[this.activeImage].big;
        },
        progressBar() {
    ...
    methods: {
        ...
        nextImage() {
            ...
            if(active >= this.imagesSrc.length) {
                active = 0;
            }
        ...
        prevImage() {
            var active = this.activeImage - 1;
            if(active < 0) {
                active = this.imagesSrc.length - 1;
            }
            this.activateImage(active);            
        },
    ...
    created() {
        ...
        if(this.startingImage 
            ...
            && this.startingImage < this.imagesSrc.length) {
...
.progressbar {
    ...
    height: 5px;
    position: absolute;
    background-color: rgba(4, 94, 249, 0.242);
    z-index: 1;
}

.progressbar > div {
    background-color: rgba(218, 23, 23, 0.52);
    height: 100%;
}
